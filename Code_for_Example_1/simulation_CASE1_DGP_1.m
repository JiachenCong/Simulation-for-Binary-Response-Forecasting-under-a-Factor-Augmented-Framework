...
for N=Number
    for T=Time
        ...
        for k=1:R
            ...
            %得到数值型的预测结果
            Result_forecast=normcdf(X*hatr);
...
disp(TTT)
writetable(TT, 'Results_CASE1_DGP_1.xlsx', 'Sheet','CASE1_DGP1','WriteRowNames', true);  
writetable(TTT, 'Results_AUC_CASE1_DGP_1.xlsx', 'Sheet','CASE1_DGP1','WriteRowNames', true);  
t1 = toc;  % 计算经过的时间
