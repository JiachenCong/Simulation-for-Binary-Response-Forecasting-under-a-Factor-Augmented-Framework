    ...
    for T=Time
        ...
        for k=1:R
            ...
            for t=1:T
                ...
                %CASE_2_DGP_1: iid logistic distribution 
                E(t+1) =  random('Logistic',0,1);
...
disp(TTT)
writetable(TT, 'Results_CASE2_DGP_1.xlsx', 'Sheet','CASE2_DGP1','WriteRowNames', true);
writetable(TTT, 'Results_AUC_CASE2_DGP_1.xlsx', 'Sheet','CASE2_DGP1','WriteRowNames', true);  
t1 = toc;  % 计算经过的时间
disp(['经过时间：', num2str(t1), '秒']);
%极大似然估计函数
function output=Fun3(X,Y,beta)
leng=length(Y);
part1=Y'*log(logsig(X*[beta(1);beta(2);beta(3);beta(4);beta(5)],0,1));
part2=(ones(leng,1)-Y)'*log(ones(leng,1)-logsig(X*[beta(1);beta(2);beta(3);beta(4);beta(5)],0,1));
output=-(part1+part2);
end
%因子估计量函数
